...
describe('improvements', () => {
    ...
    describe('GET /improvements', () => {
        ...
        it('Deve retornar 200 com a lista de melhorias', async () => {
            const response = await request(process.env.BASE_URL)
                .get('/improvements')
                .set('Authorization', `Bearer ${token}`);
    ...
    describe('GET /improvements/:id', () => {
        ...
        it('Deve retornar 200 com os detalhes da melhoria (RN025, RN026)', async () => {
            const response = await request(process.env.BASE_URL)
    ...
    describe('PUT /improvements/:id', () => {
        ...
        it('Deve retornar 200 com a melhoria atualizada (RN030)', async () => {
            const response = await request(process.env.BASE_URL)
                ...
                .set('Authorization', `Bearer ${token}`)
                .send({ description: 'Melhoria Atualizada' });

            expect(response.status).to.equal(200);
            expect(response.body.description).to.equal('Melhoria Atualizada');
        });

        it('Deve retornar 400/404 com dados inválidos (RN019)', async () => {
            const response = await request(process.env.BASE_URL)
